...
\begin{description}
  ...
  \item[Output] An overview of the artifact, motivation for the selected artifact type (guidelines), and the requirements for the artifact
\end{description}
...
\begin{description}
  ...
  \item[Input] The outlined artifact and requirements for said artifact
  \item[Resources] Literature study on peer-to-peer architectures
  \item[Controls] None
  \item[Output] Guidelines for EA, with a focus on a single part of EA, in decentralized organizations (the solution artifact)
\end{description}
...
In this iteration, one of the outlined solutions is selected and elaborated on to create guidelines for EA in decentralized organizations. These guidelines will focus on one specific area of EA. This selection is based on its applicability to the case, which is an example of a decentralized organization. 
...
\begin{description}
  ...
  \item[Input] Guidelines for EA in decentralized organizations (the solution artifact)
  ...
  \item[Output] A demonstration of the solution artifact in the case organization
\end{description}
...
In the Apply Artifact sub-activity, three separate lightweight frameworks are outlined: an ``As-Is'' framework of the case, a ``To-Be'' centralized framework based on traditional EA knowledge, and a ``To-Be'' which is developed using the solution artifact. These frameworks will focus on one specific problem area of the case.
...
\begin{description}
  ...
  \item[Input] Artifact demonstration
  \item[Resources]  Knowledge on the selected case (institution of higher education in Sweden)
...
For the Choose Evaluation Strategy sub-activity, an \textit{ex ante} evaluation strategy has been chosen to be used; meaning that ``the artifact is evaluated without being used''~\cite[Ch. 9.1]{johannessonPerjons2012}. Johanneson and Perjons suggest interviews with experts for this sub-activity, however their are no experts available in the case organization, therefore an \textit{``informed argument''} approach is instead being followed. The chosen evaluation strategy (detailed in Section~\ref{section:choose_eval}) will use the artifact's demonstration to show that the artifact can solve the explicated problem and can fulfill the defined requirements.
